PROGRAM wrf_interp
    ...
    do loop = 1, number_of_input_files
        ...
        if (ncdf4 .eqv. .true.) then
            rcode = nf_create(output_file, NF_NETCDF4, mcid)
        else
...
SUBROUTINE read_namelist(path_to_input, path_to_output, interp_levels, extrapolate, &
    ...
    logical :: leap_year
    logical :: unstagger_grid
    logical :: is_used, debug, ncdf4

    NAMELIST /io/ path_to_input, path_to_output, root_name, grid_id, start_date, leap_year, debug, ncdf4
...
SUBROUTINE def_outfile_dims(ncid, mcid, num_interp_levels, nsoil, ndims, time_ivar, &
    ...
    DO i = 1, ndims
        ...
        if(dnamei(i) == 'Time') then
            ...
            if((dnamei(i) == 'west_east_stag') .and. (unstagger_grid .eqv. .TRUE.))   wanted = .FALSE.
            if((dnamei(i) == 'south_north_stag') .and. (unstagger_grid .eqv. .TRUE.)) wanted = .FALSE.
            if(wanted .eqv. .TRUE.) then
                j = j + 1
                dnamej(j) = dnamei(i)
                dvalj(j) = dvali(i)
                rcode = nf_def_dim(mcid, dnamej(j), dvalj(j), j)
...
SUBROUTINE smooth(datain, nsmooth, ew, ns, rmsg)
    ...
    real, allocatable, dimension(:,:) :: fprint
    real :: beszero, xnu(2)
    ...
    elseif (nsmooth.le.599) then   ! weighted smoother
        ...
        nfp = 1 + 2 * idist
        if (allocated(fprint)) then
            deallocate(fprint)
        end if
        allocate(fprint(nfp, nfp))
        npsq = idist * idist
