program iowrf
    ...
    call read_args(input_file, option, iratio, box_start, box_end, bit64, debug, ncdf4)
    ...
    if (bit64) then
        if (ncdf4) then
        status = nf_create(output_file, OR(NF_64BIT_OFFSET, NF_NETCDF4), mcid)
        else
...
!                    data2 = (data(:, :, :, 1:dims_in(4)-1) + data(:,:,:,3:dims_in(4))) * .5
                else
                    data2 = data
                endif
            ELSEIF (option == '-box') THEN
...
subroutine thin_ave (ain, aou, a1, a2, a3, a4, b1, b2, b3, b4, &
    ...
    integer a1, a2, a3, a4, b1, b2, b3, b4, iratio, k4, k3, j, i, m, mm1, mm2, n, nc, nn1, nn2
    real ain(a1, a2, a3, a4), aou(b1, b2, b3, b4), sum, xmn, xmx, ymn, ymx
...
subroutine read_args(input_file, option, iratio, box_start, box_end, bit64, debug, ncdf4)
...
END SELECT
        ...
        print*, " "
                ...
                print*, " -ncdf4    : Create netCDF4 file"
        print*, " "
